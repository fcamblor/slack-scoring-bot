...
- Run `npx clasp create` then choose `api` in the list of script types : a new google app script should be created under your google account.
...
- Open your app script by running `npx clasp open`
  ...
  - In the popup, copy/paste the "Current Web app URL" : this is going to be your HTTP entrypoint for the Bot  
  ...
  |---------------|-------|---------|
  | SLACK_ACCESS_TOKEN | _let this blank for the moment_ | This is going to be useful, but later |
  | LOG_ENABLED | true | This can be helpful to see logs, particularly at the beginning |
  | SPREADSHEET_ID | _let this blank for the moment_ | This is going to be useful, but later |
...
- Activate events and fill the Google App Script's Slack Bot URL :
  <img src="./assets/Slack_Event_Subscription_configuration.png" width="800px" alt="Events subcription configuration" />
  - Wait for Google App Script challenge verification : if this is not the case, review GAS' `SLACK_CHALLENGE_ACTIVATED` project property
...
- Once done, navigate to `OAuth & Permissions` section  
  ...
  - ... and paste it into Google App Script's `SLACK_ACCESS_TOKEN` Project Properties. At the same time, disable `SLACK_ACCESS_TOKEN` flag.
...
- Run `npx clasp create` then choose `sheets` in the list of script types : a new google spreadsheet & app script should be created under your google account.
  ...
  - Open Google spreadsheet based on the created URL
